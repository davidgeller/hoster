...
- **Sign-in on the repository page** accepts username + password (plus the account's 2FA code), or a **passkey**: a "Sign in with a passkey" button appears on the sign-in dialog and on the private-repository gate whenever an account has registered a passkey for that hostname (*Settings → Account → Passkeys*). Passkeys are hostname-bound, so register one on each address you use, including custom domains. Uploading, editing, renaming, deleting, and restoring always require signing in — the repository page has its own sign-in dialog that uses the same accounts, passwords, and TOTP as the admin panel.
